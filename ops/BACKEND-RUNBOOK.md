...
`POST` and `PUT` rewrites them before validation, at the root and on every child. The title is
derived outright, in the canonical form the Java and TypeScript YAML readers compose:
...
describing the old one. A verbatim write stores the pair as given, like everything else in the
document, and what is already stored keeps a stale pair until something rewrites it.
