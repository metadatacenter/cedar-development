...
def run(args):
    ...
    if args.sources:
        ...
        if args.random_seed is not None:
            ...
            positions = random.Random(args.random_seed).sample(range(total), min(total, args.limit + max(100, args.limit // 10)))
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
                for index, (offset, page) in enumerate(pool.map(fetch_page, offsets), 1):
                    pages[offset] = page
                    if index % 20 == 0 or index == len(offsets):
                        print(f'Index pages fetched: {index}/{len(offsets)}', flush=True)
        ...
        snapshots = directory / 'fetched-sources'
        snapshots.mkdir(exist_ok=True)
        def fetch(ref):
            try:
                source = client.get_json(matrix.rest.typed_artifact_path(ref))
                (snapshots / (ref.artifact_id.rsplit('/', 1)[-1] + '.json')).write_text(json.dumps(source, ensure_ascii=False))
                return ref, source, None
            except Exception as error:
                return ref, None, str(error)
        fetch_errors = 0
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
            for index, result in enumerate(pool.map(fetch, refs), 1):
                fetched.append(result)
                fetch_errors += result[2] is not None
                if index % 200 == 0 or index == len(refs):
                    print(f'Instances fetched: {index}/{len(refs)}; failures={fetch_errors}', flush=True)
        ...
        if len(sources) != args.limit:
            raise RuntimeError(f'Requested {args.limit} readable instances but obtained {len(sources)}; snapshots and errors retained')
        (directory / 'selected-ids.json').write_text(json.dumps([ref.artifact_id for ref, _ in sources], indent=2))
        templates = {}
        template_errors = []
    ...
    try:
        ...
        for index, (ref, source) in enumerate(sources, 1):
            ...
            record, outputs, yamls = evaluate_instance(bridges, validator, templates, ref, source)
            for writer, yaml in yamls.items():
                (folder / f'{writer}.yaml').write_text(yaml)
