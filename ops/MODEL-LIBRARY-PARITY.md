...
key structure of all 81 artifacts they have both processed. Both sides of the
...
Nothing is exercised on one side only, and nothing is skipped. The four
external-identifier cases that were once TypeScript's alone are rendered by the Java
library too; the three boolean cases that made up the rest of that column are gone
with the type.
...
## The `boolean` field type — removed

**Removed**, from both model libraries and from the meta-schemas. It was a model type with no user
interface and no way to author one: the embeddable editor's `InputType` had no member for it and its
renderer no branch, so a template carrying a boolean field rendered nothing where the field should be,
and the template designer never offered it. The CEE's template parser did import the type and counted
it among the kinds carrying a default value, which is how a default was read into a component that was
never drawn.

The two libraries had also drifted apart on it in three ways at once — TypeScript wrote `type: boolean`
where Java wrote `type: boolean-field`, TypeScript flattened the labels into `trueLabel`/`falseLabel`/
`nullLabel` where Java and the JSON nest them under `labels`, and TypeScript emitted a
`datatype: xsd:boolean` that nothing else did. Closing those was what showed the type up: each library
had been refusing the other's boolean documents, which was the only thing preventing the labels from
being silently dropped on the way through.

Corpus fields 005, 006 and 007 were the only boolean artifacts in existence and went with the type. The
four `inputType` enums that listed it — literal-field, multi-instance-field, template and element — no
longer accept it.

---

## Divergences on the 81 shared artifacts
...
across `templates/004`, `009`, `037`) — **resolved: Java was right**

Recorded here as Java inventing a key, on the grounds that the source `_ui` for a page
break is `{"inputType": "page-break"}` and Java emitted
`{"inputType": "page-break", "_content": null}`. The meta-schema says otherwise:
`staticFieldUIContent` **requires** `_content` of every static field's `_ui`, so the key
is owed and the source is the thing at fault. Put through the canonical `CedarValidator`,
`template-004.json` is rejected with 72 errors, two of them naming the missing
`_content`; Java's rendering of it passes with none.

TypeScript matched the source and was rejected the same way. It now writes the key, and
its rendering of templates 4, 9 and 37 validates clean. What that costs is the verbatim
...
1. ~~**Decide on `boolean`.**~~ **Resolved: the type is removed.** It was supported by the two model
   libraries and the meta-schemas and by nothing else — no component in the embeddable editor, no
   entry in its `InputType`, no way to author one in the template designer. Rather than build the
   missing half, the type is gone from both libraries, from the four `inputType` enums, and from the
   corpus, whose fields 005-007 were the only boolean artifacts there were.
...
elements lost their question text on a YAML round trip; both writer and reader now carry them. A static
field's `width` and `height` sat among the field's own keys, where the specification puts them only for
a field written standalone — for a child they belong in that child's `configuration`, which closed
template 9. `instanceType` was modelled in TypeScript but written to and read from JSON only, so a
template's instance type was lost on every YAML round trip; the key closed element 6 and templates 7
and 35. And the `_content` entry above turned out to point the other way.
...
> **`develop`**. The `_ui._size` and `skos:prefLabel` resolutions above are deployed (release 2.9.1);
> the boolean removal and the TypeScript changes reach CEE and the Template Editor on the next npm
> release, and the meta-schema change on the next deploy of the validation library.
