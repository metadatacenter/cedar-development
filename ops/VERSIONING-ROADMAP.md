...
- **3. Author-facing version selection, in the new term picker (frontend-only).** Constraints are
   ...
   fields, described under [The Term Picker](#the-term-picker). Version selection ships as part of it. Retrofitting the old AngularJS picker (`cedar-template-editor/app/scripts/controlled-term/`) was
...
- **4. End-to-end frozen read (after CEE sends the pin, item 2).** Verify the full loop on the live stack:
...
### 5. Is the content-identity label choice correct?
...
### 6. Instance-level version capture (design decision documented)
...
- **7. Retire the ontology-constraint `sourceUri` from the model/JSON.** The YAML half is done — it is no
...
- **23. Embed it in the Template Designer.** The host integration is DOM-level: set properties, listen
...
- **24. Make the overlay behave, once there is one to behave.** The picker is an inline panel today
...
- **25. Show the pinned version in the field's configuration panel.** The panel already lists
   ...
   at neither.
- **26. Retire three capabilities cleanly: provisional creation, property search, relation types.**
...
- **27. Order across ontologies.** Ranking on the match reason is in place, which is the field half
...
- **28. Make one credential work.** The server does not agree with itself: `POST /search` and
...
- **29. Proxy the ontologies the store cannot hold.** `proxied` is a designed state the server never
...
- **30. Capture definitions at ingest.** A class hit carries no definition, because the snapshot
...
- **31. Keep the index fresh.** A re-ingest moves an ontology's current version and the index does
...
- **32. Ship behind a flag for one release, then delete what it replaces.** The new component is the
