...
- **29. Resolve the remaining production artifact defects and review semantic migrations.**
  ...
  **Resolve the remaining production instance sources and pipeline findings.** The retained
  findings and checked repairs leave 51 affected instances.
  ...
  to the 51-instance baseline. Account for their stricter-reader rejection before production
  ...
  | Malformed/empty stored URI is the first Java rejection | 26 |
  ...
  its dependent-instance blockers. All 368 instances require object-to-array migration; 359
  proposed bodies validate, while nine retain existing errors (eight country IRIs and one source
  link with four numeric-value errors). Do not switch the template before those instances
  can be migrated. This approved repair remains within the 51-instance backlog; its dependencies
  ...
  `$CEDAR_HOME/.cedar/repairs/2026-09-26-webmanagercenter-source-link/`, plus the two-property
  repair under `$CEDAR_HOME/.cedar/repairs/2026-09-26-a147-link-numeric/`. Refresh the full
